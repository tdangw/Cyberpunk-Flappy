...
export class CollisionSystem {
  ...
  public checkCollisions(ctx: CollisionContext): {
          ...
          if (bird.isInvulnerable()) {
            ...
            if (Math.random() > 0.8) {
              window.dispatchEvent(
                new CustomEvent('shieldActive', {
                  detail: { x: bird.x, y: bird.y },
                })
              );
            }
    ...
    for (let i = 0; i < coins.length; i++) {
      ...
      if (
        bird.x + bird.radius + 5 < coin.x - coin.r ||
        bird.x - bird.radius - 5 > coin.x + coin.r
      )
        continue;
    ...
    for (let i = 0; i < enemies.length; i++) {
      ...
      if (
        birdRect.r > enemyRect.l &&
        birdRect.l < enemyRect.r &&
        birdRect.b > enemyRect.t &&
        birdRect.t < enemyRect.b
      ) {
        // STOMP LOGIC
        ...
        if (isNotJumpingUp && hitTopPart) {
          ...
          window.dispatchEvent(
            new CustomEvent('enemyStomp', {
              detail: { x: enemy.x, y: enemy.y },
            })
          );
          continue;
        }
