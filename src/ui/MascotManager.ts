...
export class MascotManager {
    ...
    private rotZ = 0;
    private rotX = 0;
    private state: 'loading' | 'flying' | 'idle' | 'acting' | 'falling' | 'dizzy' | 'sleeping' | 'dragging' = 'loading';
    ...
    private onDragMove(e: MouseEvent | TouchEvent) {
        ...
        if (Math.abs(clientX - this.dragStart.x) > 5 || Math.abs(clientY - this.dragStart.y) > 5) {
            this.hasMoved = true;
    ...
    private handleDrop() {
        ...
        const isOverButton =
            mascotCX >= btnRect.left &&
            mascotCX <= btnRect.right;
    ...
    updatePosition(progress: number) {
        ...
        if (!this.initialized) {
            ...
            this.container.style.transition = "left 0.1s linear, top 0.1s linear, opacity 0.5s ease-out";
    ...
    private flyToButton() {
        ...
        const animate = (timestamp: number) => {
            ...
            const nextY = startY + (endY - startY) * ease - Math.sin(p * Math.PI) * 150;
    ...
    private startIdle() {
        ...
        const loop = () => {
            ...
            if (rand < 0.50) {
                ...
                this.rotY = nextX > this.pos.x ? 0 : 180;
                this.pos.x = nextX;
                this.container.style.transition = "left 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
                this.apply();
                setTimeout(() => { if (this.state === 'idle') this.container.style.transition = "none"; }, 600);
            }
    ...
    private triggerRandomReaction() {
        ...
        else if (rand < 0.9) this.actionSing(1000); // Short sing
    ...
    private actionFallEpic() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                this.stars.style.display = 'none';
                this.container.classList.remove('dizzy-eyes', 'stun-active', 'upside-down-stars');
        ...
        */
        ...
        setTimeout(() => {
            ...
            requestAnimationFrame(() => {
                this.container.classList.add('flapping');
                this.container.style.transition = "top 1.0s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
                this.pos.y = landY;
                this.apply();
