...
export class HUD {
  ...
  private updateControlUI(): void {
    ...
    else if (config.dashControl === 'button_left')
  ...
  showGameOver(
    ...
    bestDist: number = 0,
    canAdRevive: boolean = false,
    canQuickRevive: boolean = false
  ): void {
    ...
    if (b)
      b.textContent = this.ui.saveManager.getHighScore(isClassic).toString();
    ...
    if (themeNameEl && hudLabel) {
      ...
      setTimeout(() => {
        hudLabel.style.transition =
          'transform 2s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.5s';
  ...
  replaceIcons(): void {
    const setIcon = (id: string, type: any) => {
      ...
      if (el)
        el.innerHTML = `<img src="${IconDrawer.getSimpleIcon(type)}" style="width: 70%; height: 70%; object-fit: contain; pointer-events: none;">`;
    };
  ...
  private setupTooltips(): void {
    ...
    icons.forEach((icon) => {
      ...
      el.addEventListener(
        'mouseenter',
        (e) => this.ui.showTooltip(icon.text, e.clientX, e.clientY),
        { signal }
      );
